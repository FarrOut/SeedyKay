...
from motley.CICD.cross_account.cross_account_codepipeline_stack import CrossAccountCodePipelineStack
...
security_groups = SecurityGroupsStack(app, "SecurityGroupsStack",
                                      vpc=net.vpc,
                                      env=Environment(account=os.getenv('CDK_DEFAULT_ACCOUNT'),
                                                      region=os.getenv('CDK_DEFAULT_REGION')),
                                      )
...
imported_eks = ImportedEksStack(app, "ImportedEksStack",
                                vpc=net.vpc,
                                cluster=eks.cluster,
...
kinesis = KinesisStack(app, "KinesisStack",
                       env=Environment(account=os.getenv('CDK_DEFAULT_ACCOUNT'),
                                       region=os.getenv('CDK_DEFAULT_REGION')),
                       )
...
scheduler = SchedulerStack(app, "SchedulerStack",
                           env=Environment(account=os.getenv('CDK_DEFAULT_ACCOUNT'),
                                           region=os.getenv('CDK_DEFAULT_REGION')),
                           )

r53 = Route53Stack(app, "Route53Stack",
                   vpc=net.vpc,
...
logs = LogGroupStack(app, "LogGroupStack",
                     env=Environment(account=os.getenv('CDK_DEFAULT_ACCOUNT'),
                                     region=os.getenv('CDK_DEFAULT_REGION')),
                     )

elasticache = ElastiCacheStack(app, "ElastiCacheStack",
                               vpc=net.vpc,
...
ad = DirectoryStack(app, "DirectoryStack",
                    vpc=net.vpc,
...
cross_account_pipeline = CrossAccountCodePipelineStack(app, "CrossAccountCodePipelineStack",
